...
cdef class GrowSeedAlgo(object):
    '''
        only for binary image
    '''
    cdef Py_ssize_t im_height
    cdef Py_ssize_t im_width
    cdef int max_label
    cdef int label_id
    cdef np.ndarray im_label
    cdef np.ndarray im
    cdef object stack
    cdef object white_line_list
    cdef object white_line
    cdef object node_list


    def __init__(self, node_list,np.ndarray im=None, im_path=None,):
        print(im)
        print(type(im))
        self.stack = Stack()
    ...
    def _get_neighbour(self, int x0,int y0):
        neighbour = []
        for i in range(-3,4):
            for j in range(-3,4):
                if (i,j) == (0,0): 
    ...
    def _in_region(self,int x,int y):
        return True if 0<=x<self.im_height and 0<=y<self.im_width else False
    
    def node_condition(self, int x, int y):
