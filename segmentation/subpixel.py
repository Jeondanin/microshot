...
def icnr_weights(init = tf.keras.initializers.GlorotNormal(seed=None), scale=2, shape=[3,3,32,4], dtype = tf.float32):
    hello = ICNR(init, scale=scale)(shape=shape, dtype=dtype)
    def wrapper(x, c):
      return c
    f = tf.function(wrapper)
    print(hello)
    return eval(f([],hello))
